...
function AddCoins() {
    const {coins, addCoins, removeCoins}= useCoin();
    const [amount, setAmount] = useState('');
    
  return (
  <section className='text-center flex justify-center items-center h-[60vh]'>  
    <div className="container py-10 px-7 h-[80%] shadow-lg w-[30%] space-y-4 border border-gray-300 rounded-lg bg-white">
        ...
        <p className='p1 text-sm '>Add virtual coins to your account to donate to campaigns.</p>
        </div>
        ...
        <div className="btn w-full flex justify-around space-x-4 px-2">
            <button onClick={()=>setAmount('')} className="cancel button w-1/2 !bg-gray-200 shadow-md !text-black hover:!bg-gray-300 hover:!border-0">Cancel</button>
            <button onClick={()=>addCoins(amount)} className="add button shadow-md w-1/2">Add Coins</button>
        </div>
